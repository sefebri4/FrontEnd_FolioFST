...
class DosenController extends Controller
{
    ...
    // Dosen Informatika
    public function dosenIf()
    {
        $dosen_if = Dosen::with('programStudi')
            ->whereHas('programStudi', function ($q) {
                $q->where('nama', 'like', '%Informatika%');
            })->get();

        return view('dosen_if', compact('dosen_if'));
    }

    // Dosen RPL
    public function dosenRpl()
    {
        $dosen_rpl = Dosen::with('programStudi')
            ->whereHas('programStudi', function ($q) {
                $q->where('nama', 'like', '%Rekayasa Perangkat Lunak%');
            })->get();

        return view('dosen_rpl', compact('dosen_rpl'));
    }

    // Dosen Sistem Informasi
    public function dosenSi()
    {
        $dosen_si = Dosen::with('programStudi')
            ->whereHas('programStudi', function ($q) {
                $q->where('nama', 'like', '%Sistem Informasi%');
            })->get();

        return view('dosen_si', compact('dosen_si'));
    }

    // Detail biodata dosen (halaman publik)
    public function show($id)
    {
        $dosen = Dosen::with('programStudi')->findOrFail($id);
        return view('biodata_dosen', compact('dosen'));
    }
